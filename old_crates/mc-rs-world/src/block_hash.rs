...
/// Legacy bedrock hash produced by empty `states {}` serialization.
///
/// Kept for compatibility with already-persisted chunks generated by older
/// mc-rs builds. Canonical bedrock uses `infiniburn_bit`.
pub const LEGACY_BEDROCK_HASH_EMPTY_STATES: u32 = 0x0FF5_9FE6;

/// Compute bedrock hash with explicit `infiniburn_bit` state.
pub fn hash_bedrock_state(infiniburn: bool) -> u32 {
    hash_block_state_with_props(
        "minecraft:bedrock",
        &[(
            "infiniburn_bit",
            StateValue::Byte(if infiniburn { 1 } else { 0 }),
        )],
    )
}

/// Canonical default bedrock state used by PocketMine/Bedrock data.
pub fn hash_default_bedrock() -> u32 {
    hash_bedrock_state(false)
}

/// Map legacy empty-states bedrock hash to canonical default bedrock hash.
...
mod tests {
    ...
    #[test]
    fn bedrock_hash_matches_canonical_default_state() {
        assert_eq!(hash_default_bedrock(), 0xC411_A083);
        assert_eq!(hash_bedrock_state(true), 0x20AE_D45E);
    }

    #[test]
    fn legacy_bedrock_hash_is_normalized() {
        let legacy = hash_block_state("minecraft:bedrock");
        assert_eq!(legacy, LEGACY_BEDROCK_HASH_EMPTY_STATES);
        assert_ne!(legacy, hash_default_bedrock());
        assert_eq!(
            normalize_legacy_bedrock_hash(legacy),
            hash_default_bedrock()
        );
    }
