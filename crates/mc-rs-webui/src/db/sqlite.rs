//! Impl SQLite via `sqlx`. Feature `sqlite` (default).
...
impl SqliteDb {
    /// `path` est le chemin fichier après `sqlite://` (ex: "webui.db").
    pub async fn open(path: &str) -> Result<Self> {
        let opts = SqliteConnectOptions::from_str(path)
            .map_err(|e| Error::Db(format!("invalid sqlite path '{path}': {e}")))?
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .connect_with(opts)
            .await
            .map_err(|e| Error::Db(format!("sqlite connect: {e}")))?;
...
impl WebDb for SqliteDb {
    ...
    async fn audit_page(
        ...
        // Construction dynamique mais paramètrée — pas de SQL injection possible.
    ...
    async fn blacklist_token(&self, jti: &str, exp_unix: i64) -> Result<()> {
        sqlx::query("INSERT OR REPLACE INTO tokens_blacklist (jti, exp) VALUES (?, ?)")
            .bind(jti)
            .bind(exp_unix)
            .execute(&self.pool)
            .await
    ...
    async fn cleanup_expired_tokens(&self, now_unix: i64) -> Result<u64> {
        let res = sqlx::query("DELETE FROM tokens_blacklist WHERE exp < ?")
            .bind(now_unix)
            .execute(&self.pool)
            .await
            .map_err(|e| Error::Db(e.to_string()))?;
